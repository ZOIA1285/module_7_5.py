...
import time  # Импортируем модуль time для корректного отображения времени

# Указываем директорию для обхода каталога(текущая директория)
directory = "."
...
for root, dirs, files in os.walk(directory):
    for file in files:  # Перебираем все файлы в текущем каталоге
        ...
        print(f'Обнаружен файл: {file}, Путь: {filepath}, Размер: {filesize} байт, Время изменения: '
              f'{formatted_time}, ' f'Родительская директория: {parent_dir}')